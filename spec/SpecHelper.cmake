...
function(build_spec_target)
    set(options)
    set(oneValueArgs PROJECT TARGET PATH RESULT OUTPUT SAFE_OUTPUT)
    ...
    if(DEFINED ${arg_prefix}SAFE_OUTPUT)
        string(REGEX REPLACE "\\\\" "/" safe_output "${try_compile_output}")
        set(${${arg_prefix}SAFE_OUTPUT} "${safe_output}" PARENT_SCOPE)
    endif()
endfunction()

function(add_file_or_folder_to_cleanup folder_path)
    set_property(GLOBAL APPEND PROPERTY CREATED_PROJECT_FOLDERS "${folder_path}")
endfunction()

function(create_temp_directory out)
    get_temp_directory(path)
    add_file_or_folder_to_cleanup("${path}")
...
function(create_project out_folder_path content)
    ...
    get_filename_component(filename "${CSPEC_FILE}" NAME_WLE)
    string(SHA1 file_id "${CSPEC_FILE}")
    string(RANDOM random)
    string(TIMESTAMP timestamp "%m-%d-%Y_%H-%M-%S_%f")
    set(project_folder "$ENV{TEMP}/SkyrimScripting.CMake/Specs/${filename}-${file_id}/${CSPEC_TEST_FUNCTION}/${timestamp}-${random}")
    ...
    add_file_or_folder_to_cleanup("${project_folder}")
endfunction()

function(teardown_cleanup_generated_projects)
    if(NOT DEFINED NO_PROJECT_CLEANUP)
        get_property(created_folders GLOBAL PROPERTY CREATED_PROJECT_FOLDERS)
        foreach(folder ${created_folders})
            message("CLEANUP ${folder}")
